...
class MathFunctions:
    ...
    def auto_fermat_prime_test(cls, num: int, rounds: int, verbose: bool = False):
        ...
        for x in range(rounds):
            ...
            if res != 1:
                is_prime = False
            result_list.append({"testvalue": tester, "result": res})
        if is_prime is None:
            is_prime = True
    ...
    def euclid_gcd(cls, num_a: int, num_b: int):
        """Euclidean algorithm for the Greatest Common Divisor of two numbers."""
        if num_a == 0:
            return num_b
    ...
    def crt(cls, dic):
        ...
        For the example problem the input variable would be formatted like this:
            dic = [{"modulus": 9, "result": 8},{"modulus": 3, "result": 5}]
        """
        M = 1
        temp = 0
    ...
    def eea(cls, modulus: int, number: int, verbose: bool = False):
        """Extended Euclidean Algorithm with optional graphical solution."""
        if verbose:
...
class EEA:
    ...
    def _format_ascii_cell_left(self, row: int):
        return (
            f"{self.table[row][0]} = {self.table[row-2][0]}-({self.table[row-1][0]}*{self.table[row-1][1]})"
        )
